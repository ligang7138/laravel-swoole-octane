  ...
  <div class="emergency-list">
    ...
    <el-card class="table-card">
      ...
      <el-table v-loading="loading" :data="tableData" border stripe>
        ...
        <el-table-column prop="type_name" label="应急类型" width="120" align="center" />
        ...
        <el-table-column prop="process_status" label="状态" width="100" align="center">
        ...
        <el-table-column prop="add_time" label="上报时间" width="180" align="center" />
    ...
    >
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="应急类型">{{ detail.type_name }}</el-descriptions-item>
        <el-descriptions-item label="状态">
          <el-tag :type="detail.process_status === 1 ? 'success' : 'warning'">
            {{ detail.process_status_text }}
          </el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="联系人">{{ detail.linkman }}</el-descriptions-item>
        <el-descriptions-item label="联系电话">{{ detail.mobile }}</el-descriptions-item>
        <el-descriptions-item label="上报时间">{{ detail.add_time }}</el-descriptions-item>
      ...
      <div v-if="detail.process_status === 1" style="margin-top: 20px">
        ...
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="处理备注">{{ detail.process_remark || '-' }}</el-descriptions-item>
        </el-descriptions>
      </div>
...
async function fetchTypeOptions() {
  try {
    const { data } = await getEmergencyTypeList({ status: 1, page_size: 100 })
    typeOptions.value = data.list || []
  } catch (error) {
    console.error('获取应急类型失败:', error)
  }
}
...
async function fetchData() {
  ...
  try {
    ...
    const { data } = await getEmergencyList(params)
...
async function handleView(row) {
  try {
    const { data } = await getEmergencyDetail(row.id)
    detail.value = data
    detailDialogVisible.value = true
  } catch (error) {
    ElMessage.error('获取应急事件详情失败')
  }
}
...
async function handleProcessSubmit() {
  try {
    await ElMessageBox.confirm('确定处理该应急事件吗？', '提示', { type: 'warning' })
    processLoading.value = true
    await processEmergency(detail.value.id, { remark: processForm.remark })
